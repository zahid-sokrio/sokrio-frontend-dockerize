<template>
  <lazy-dropdown :collapseTags="collapseTags"
                 :disabled="disabled"
                 :end-point="endPoint"
                 :label="label"
                 :name="name"
                 ...
                 :rules="rules"
                 :value="value"
                 resource-key="departmentCategories"
                 :searchable-key="searchableKey"
                 @input="$emit('input', $event)">
<!--    <template v-slot:createModal="{isActive, afterCreated}">-->
<!--&lt;!&ndash;      <create-user v-if="isActive" :is-active.sync="isActive" @created="afterCreated($event)"></create-user>&ndash;&gt;-->
<!--    </template>-->
  </lazy-dropdown>
</template>
<script lang="ts">
...
export default class DepartmentCategoryLazyDropdown extends Mixins(DropdownField) {
  @Prop({default: 'department-categories'})

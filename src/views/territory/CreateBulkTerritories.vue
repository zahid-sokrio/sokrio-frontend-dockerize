<template>
  <modal :show="showBulkTerritoriesModal" @close="close">
    <template slot="header">
      <h5 class="modal-title"> Add Bulk Territories</h5>
    </template>
    <!-- File browser -->
    <div class="card">
      <!-- Card header -->
...
@Component({
  name: 'CreateBulkTerritories',
  components: {
    FileInput
  }
})
export default class CreateBulkTerritories extends Vue {
  private btnDisabled: boolean = false;
  private csvFile: Blob|string = '';
  private errors: Array<string> = [];

  @Prop() showBulkTerritoriesModal!: boolean;

  close() {
    this.errors = [];
    this.$emit('update:showBulkTerritoriesModal', false);
  }

  _import() {
    this.btnDisabled = true;
    let form = new FormData();
    form.append('territories', this.csvFile);
    this.$http.post(this.$api('/import-territories'), form)

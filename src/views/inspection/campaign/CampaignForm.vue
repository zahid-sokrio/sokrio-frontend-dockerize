    ...
    <validation-observer v-slot="{handleSubmit}" ref="formValidator">
        <modal :show="isActive" v-if="isActive" @close="close">
            <template slot="header">
                ...
                >{{isUpdate ? 'Update' : 'Add New'}} Merchandise</h5>
            </template>
            <div class="col-md-12">
                <base-input prepend-icon="ni ni-box-2" v-model="form.name" label="Name" name="name" placeholder="e.g. Sticker"></base-input>
...
<script lang="ts">
    ...
    export default class AssetForm extends Mixins(BaseCUModalComponentMixin){
        ...
        async afterCreate(response: AxiosResponse<any>): Promise<void> {
            this.$emit('created', response.data.campaignItem);
        }

        get updateApi(): string {
            return `/campaign-items/${this.currentEntity.id}`
        }

        afterUpdate(response: AxiosResponse<any>): void {
            this.$emit('updated', response.data.campaignItem);
        }
    }
</script>

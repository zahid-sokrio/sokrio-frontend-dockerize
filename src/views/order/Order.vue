    ...
    <base-header type="info">
      <div class="row align-items-center py-4">
        <div class="col-lg-6 col-7">
          <h6 class="h2 text-white d-inline-block mb-0"> {{ getTaxonomy('order') }} </h6>
      ...
      <div class="col">
        <ul class="nav nav-tabs" id="profile-tab" role="tablist">
          <template v-for="(route, index) in OrderRoutes.children">
...
<!--                  {{ ['Orders', 'Requisitions', 'Sales Return', 'Invoices'].includes(route.meta.label) ? getTaxonomy( route.meta.label == 'Orders' ? 'order' : route.meta.label == 'Sales Return' ? 'salesReturn' : route.name.slice(0, -1) ) : route.meta.label }}-->
                  {{route.meta.label}}
                </span>
              </router-link>
            </li>

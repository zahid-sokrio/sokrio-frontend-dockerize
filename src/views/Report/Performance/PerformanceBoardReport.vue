<template>
  <div class="row pl-3" v-can="permissionEnum.PERFORMANCE_BOARD_REPORT">
    <div class="col-12">
      <div class="card">
        <el-table
          v-loading="loading"
        ...
        >
          <el-table-column label="Target Name" min-width="150px" prop="type" align="center" sortable>
            <template v-slot="{ row }">
              <b>{{ row.target_name }}</b>
            </template>
          </el-table-column>
    ...
    <modal :show="showDownloadModal" @close="showDownloadModal = false">
      <template slot="header">
        <h5 class="modal-title">Download Performance Board Report</h5>
      </template>
      <div class="card">
        <div class="card-body">
  ...
  async downloadPerformanceBoard(status: string) {
    ...
    try {
      ...
      if (e.response.status === 422) {
        await swal.fire('Sorry!', e.response.data.errors.message[0], 'warning');
      } else {
